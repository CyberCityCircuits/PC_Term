...
def process_write_id_chk(dept, value):
    ...
    if value == 1:
        ...
        subchild = ('<idChecks><domain:idCheck sysid="2"/></idChecks>')
    else:
        header()
    ...
    for department in soup.find_all('department'):
        ...
        if xmlin_dept == dept:
            x+=1
            department.find_parent('PLU').append(str(subchild))
    ...
    print ((str(x) + " ID Checks added to Department " + str(dept)).center(cent_width))
