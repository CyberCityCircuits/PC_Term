...
menu_07 = "7 - Reset All Food Stamp Items"
...
#set flags by dept  (sysid=4 for fs)
def process_write_flags(dept,value):
    ...
    for department in soup.find_all('department'):
        ...
        if xmlin_dept == dept:
            ...
            sysid = (department.find_parent('PLU').flags.contents)
            #this checks to see if any flags are in the flags subchild
            if not sysid:
                print ("Currently No Flags.  Adding Food Stamp.".center(cent_width))
                #This line will add the flag_value tag to the flags subchild
                department.find_parent('PLU').flags.append(soup.new_tag(flag_value))
            else:
                #drops the first and last items from sysid (usually blank lines)
                sysid.pop(-1)
                sysid.pop(0)
                
            print (sysid)
            
            #since there may be mulptible items in the flag subchild 
            #we need to check to see if the desired value already exists.
                
            
            if sysid.count('<domain:flag sysid="4"/>') >= 1:
    ...
    f = open(dir_temp + "\\" + log_name + ".txt","a")
    f.write("Writing Flags " + currdate + " " + currtime + "\n")
    f.write("Department - " + str(dept) + "\n")
    f.write("Value - " + str(value) + "\n")
    ...
    print ("  " + str(x) + " Flags Added to Department " + str(dept))
...
def set_alcohol_ID():
    ...
    if add_id_val_chk:
        ...
        #0 Completes Input Process
            ...
            if option.lower( )== ("y"):
                #file_idchk = (dir_temp + "/" + plu_xml)
                #subchild_Alcohol = '<idChecks>\n<domain:idCheck sysid="1"/>\n</idChecks>'
                #subchild_Alcohol = id_checks.pop(0)
                #print (subchild_Alcohol)
                #os.system("pause")
