...
def process_remove_fs():
    ...
    #data = soup.get_text()
    #soup2 = data.replace('\n', '')
    #soup2 = "".join(line.strip() for line in soup.split("\n"))
    #soup2 = soup.strip()
    ...
    plu_new = open(dir_temp + "\\" + plu_xml,"w")
    plu_new.write(new_data)
    plu_new.close()
    
    ####WRITES THE NEW PLUs.xml FILE
    #with open(outfile, 'w') as xmlout:
    #    xmlout.write(soup.prettify())
        
    #xmlout.close()
    
    #with open(outfile,'r') as data_raw:
    #    data = data_raw.readlines()
    #    print (data)
    #    os.system('pause')
        #data = data_raw.read().replace('\n', '')
    #    data = data_raw.read()
    #    data = data.replace('/n', '')
    #    data = data.strip()
    #    print (str(data))
